...
| D012 | Ignorar `Atendente.Email` e `Atendente.Telefone` no mapping EF atual. | Accepted | As migrations legadas de `Atendentes` nao possuem essas colunas e SQL Server real expôs `Telefone` como tipo sem chave no EF Core 10. |
